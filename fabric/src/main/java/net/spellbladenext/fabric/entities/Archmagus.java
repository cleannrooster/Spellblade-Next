...
public class Archmagus extends PathfinderMob implements InventoryCarrier, IAnimatable {
    ...
    public static final RawAnimation DOWN = new RawAnimation("animation.hexblade.down", ILoopType.EDefaultLoopTypes.HOLD_ON_LAST_FRAME);
    ...
    public static final EntityDataAccessor<Boolean> BIDED;

    static {
        TIER = SynchedEntityData.defineId(Archmagus.class, EntityDataSerializers.INT);
        modifier = SynchedEntityData.defineId(Archmagus.class, EntityDataSerializers.INT);

        FLOATING = SynchedEntityData.defineId(Archmagus.class, EntityDataSerializers.BOOLEAN);
        RAISING = SynchedEntityData.defineId(Archmagus.class, EntityDataSerializers.BOOLEAN);
        FLYING = SynchedEntityData.defineId(Archmagus.class, EntityDataSerializers.BOOLEAN);
        JUMPING = SynchedEntityData.defineId(Archmagus.class, EntityDataSerializers.BOOLEAN);
        BIDED = SynchedEntityData.defineId(Archmagus.class, EntityDataSerializers.BOOLEAN);
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(BIDED, false);
    ...
    public void addAdditionalSaveData(CompoundTag compoundTag) {
        ...
        compoundTag.putBoolean("biding",this.entityData.get(BIDED));
    ...
    public void readAdditionalSaveData(CompoundTag compoundTag) {
        ...
        if (compoundTag.contains("biding")) {
            this.entityData.set(BIDED, compoundTag.getBoolean("biding"));
        }
    ...
    public Archmagus(EntityType<? extends Archmagus> p_34652_, Level p_34653_) {
        super(p_34652_, p_34653_);
    }

    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 600.0D).add(Attributes.MOVEMENT_SPEED, 0.3499999940395355D).add(Attributes.ATTACK_DAMAGE, 8.0D).add(Attributes.KNOCKBACK_RESISTANCE,0.5);
    }
    @Override
    ...
    public boolean hurt(DamageSource damageSource, float f) {
        if(damageSource.getEntity() instanceof Player player && player.getMainHandItem().getItem() instanceof DebugNetherPortal){
            return super.hurt(damageSource,this.getMaxHealth()*0.55F);
        }
    ...
    protected void tickDeath() {
        if(this.getServer() != null && this.getServer().getLevel(Level.OVERWORLD) != null && (this.getServer().getLevel(Level.OVERWORLD).isThundering() || this.getServer().getLevel(Level.OVERWORLD).isRaining())) {

            this.getServer().getLevel(Level.OVERWORLD).setWeatherParameters(0, 12000, false, false);
        }
        super.tickDeath();
    }
    ...
    public void tick() {
        super.tick();
        this.biding = this.getHealth() < this.getMaxHealth() / 2 && !this.getEntityData().get(BIDED);
        if(this.getEntityData().get(BIDED)){
            if(this.getServer() != null && this.getServer().getLevel(Level.OVERWORLD) != null &&!this.getServer().getLevel(Level.OVERWORLD).isThundering()){
                this.getServer().getLevel(Level.OVERWORLD).setWeatherParameters(0,12000,true,true);

            }
        }
        if(this.biding){
            if(this.getServer() != null && this.getServer().getLevel(Level.OVERWORLD) != null &&!this.getServer().getLevel(Level.OVERWORLD).isRaining()){
                this.getServer().getLevel(Level.OVERWORLD).setWeatherParameters(0,12000,true,false);
            ...
            if(this.bidingtime < 20*30){
                ...
                this.getEntityData().set(BIDED,true);
                this.biding = false;
            }
            if(this.tickCount % (20 * 5) == 0 && !this.getLevel().isClientSide()){
                SpellbladesFabric.attackeventArrayList.add(new attackevent(this.getLevel(),this.getLevel().getNearestPlayer(this,16)));
            }

        }
        if(this.specialattack){
            if(this.specialattacktime == 10 && !this.getLevel().isClientSide() && !this.shotlightning){
        ...
        if(this.tickCount % 5 == 0 && this.getLevel().isClientSide){
            ...
            players.forEach(player -> {
                player.addEffect(new MobEffectInstance(SpellbladesFabric.PORTALSICKNESS.get(),160,0,false,false));
                String string = "null";
        ...
        if (this.tickCount % 5 == 0 && this.getLevel() instanceof ServerLevel level) {
            ...
            List<Entity> magi =  StreamSupport.stream(level.getAllEntities().spliterator(),true).filter(entity -> entity instanceof Archmagus).toList();
    ...
    protected Brain<?> makeBrain(Dynamic<?> dynamic) {
        return ArchmagusAI.makeBrain(this,brainProvider().makeBrain(dynamic));
    }
    protected Brain.Provider<Archmagus> brainProvider() {
    ...
    public void swing(InteractionHand interactionHand, boolean bl) {
        if (!this.swinging || this.swingTime >= 18 || this.swingTime < 0) {
            ...
            this.swingTime = -1;
            if(this.getRandom().nextInt(4)==0 && !this.specialattack && this.getEntityData().get(BIDED)){
    ...
    private <E extends IAnimatable> PlayState predicate2(AnimationEvent<E> event) {
        if(this.biding){
            ...
            animations.getRawAnimationList().add(DOWN);
    ...
    public void registerControllers(AnimationData animationData) {
        animationData.addAnimationController(new AnimationController<Archmagus>(this,"walk",0,this::predicate2));

        animationData.addAnimationController(new AnimationController<Archmagus>(this,"attack",0,this::predicate));
